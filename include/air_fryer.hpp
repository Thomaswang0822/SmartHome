#pragma once

#include "device.hpp"
#include "utils.hpp"

#include <assert.h>

/// @brief TODO: Add "AirFryer Concurrency":
/// 0  min: add fish that takes 20 mins;
/// 5  min: add chicken wings that takes 10 mins;
/// 15 min: chicken wings ready;
/// 20 min: fish ready;
class AirFryer : public Device<AirFryer, AirFryerData> {
public:
    /// @brief
    /// @param volume How big is the AirFryer in liter.
    AirFryer(float volume = 5.f) : Device("AirFryer"), k_total_volume(volume), m_volume(volume) {};
    ~AirFryer() = default;

    void implOperate(std::shared_ptr<AirFryerData> data);
    void implMalfunction(std::shared_ptr<AirFryerData> data);
    uint32_t implTimeTravel(const uint32_t duration_sec);

private:
    // Data
    const float k_total_volume;
    float m_volume;

    // Functions
    void cook(std::shared_ptr<AirFryerData> data);
    void cleanup(std::shared_ptr<AirFryerData> data);
};
