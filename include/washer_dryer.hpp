...
class WasherDryer : public Device<WasherDryer, WasherDryerData> {
public:
...
private:
    ...
    Timer m_wash_timer = {};
    Timer m_dry_timer = {};
    std::deque<std::shared_ptr<WasherDryerData>> m_wash_bin;
    std::deque<std::shared_ptr<WasherDryerData>> m_dry_bin;
    ...
    void wash(std::shared_ptr<WasherDryerData> data);

    /// @brief Async Dry operation, works the same as `Wash()` except for step 3.
    void dry(std::shared_ptr<WasherDryerData> data);
